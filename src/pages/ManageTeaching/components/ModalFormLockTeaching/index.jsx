...
export function ModalFormLockTeaching({ onSuccess, openForm, onChangeClickOpen }) {
  const handleLockTeachingList = (values) => {
    lockTeachingList(values).then((res) => {
  ...
  return (
    <div>
      <ModalForm
        width={700}
        title={'Khóa danh sách giảng dạy và điểm'}
        modalProps={{
          maskClosable: false,
          destroyOnClose: true,
          okText: 'Đồng ý',
        ...
        open={openForm}
        onFinish={(values) => handleLockTeachingList(values)}
